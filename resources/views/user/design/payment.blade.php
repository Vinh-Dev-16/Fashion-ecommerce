                            ...
                            <div class="sumary_totals">
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        <span class="total">{{ number_format($subTotal + 15000+ $subTotal * 0.1) }} VND</span>
                                    </li>
                                </ul>
                            </div>
...
<script>
    ...
    let voucher = document.querySelector('#voucher_item');
     voucher.addEventListener('change', (e) => {
        if(!(e.target.value == 'Chọn voucher')){
    ...
    function showVoucher(data) {
        ...
        if(data.result[0].voucher > 100){
            total.innerText = ((caculator + (caculator * 0.1) + ship) - data.result[0].voucher).toLocaleString('vi-VN') + ' VND';
        }else{
            total.innerText = (Math.floor((caculator + (caculator * 0.1) + ship)-((caculator + (caculator * 0.1) + ship) * (data.result[0].voucher /100)))).toLocaleString('vi-VN') + ' VND';
        }
    }
</script>
@endsection